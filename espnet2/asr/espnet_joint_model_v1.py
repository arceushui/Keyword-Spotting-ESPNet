...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        #speech_mix = torch.tensor(speech_mix, device= speech_mix.device, dtype=torch.float64)
        #speech_mix_lengths = torch.tensor(speech_mix_lengths, device=speech_mix_lengths.device, dtype=torch.float64)
    ...
    def inference(
        ...
        **kwargs,
        ...
        # 1. Encoder
        #logging.info(f"in the inference fucntion in espnet_joint_model3.py,predicted_magnitude is {predicted_magnitude} its shape is {predicted_magnitude[0].shape}")
        #logging.info(f"in the inference fucntion in espnet_joint_model3.py, tf_length is {tf_length}")
