...
class Trainer:
    ...
    def train_one_epoch(
        ...
        #for k, p in model.named_parameters():
            #if k.startswith("separator") or k.startswith("enh_model"):
            #    logging.info(f"in the train_one_epoch function, model parameter is k {k} and p is {p}")
        ...
        #for iiter, (uttid, batch) in enumerate(
            reporter.measure_iter_time(iterator, "iter_time"), 1
        ):
            assert isinstance(batch, dict), type(batch)
            #logging.info(f"in the train , uttid is {uttid} and batch is {batch}")
            if distributed:
            ...
            with autocast(scaler is not None):
                with reporter.measure_time("forward_time"):
                    loss, stats, weight = model(**batch) # md add
                    #loss, stats, weight = model(uttid,**batch)
    ...
    def validate_one_epoch(
        ...
        #for k, p in model.named_parameters():
        #    if k.startswith("separator") or k.startswith("enh_model"):
        #        logging.info(f"in the validate_one_epoch function, model parameter is k {k} and p is {p}")

        # [For distributed] Because iteration counts are not always equals between
        # processes, send stop-flag to the other processes if iterator is finished
        iterator_stop = torch.tensor(0).to("cuda" if ngpu > 0 else "cpu")
        for (_, batch) in iterator:
        #for (uttid, batch) in iterator:
            assert isinstance(batch, dict), type(batch)
            #logging.info(f"in the valid , uttid is {uttid} and batch is {batch}")
            if distributed:
                torch.distributed.all_reduce(iterator_stop, ReduceOp.SUM)
                if iterator_stop > 0:
                    break
            ...
            #_, stats, weight = model(uttid, **batch)
            if ngpu > 1 or distributed:
    ...
    def plot_attention(
        ...
        #for ids, (uttid, batch) in iterator:
            logging.info(f"ids is {ids}, batch is {batch}")
            assert isinstance(batch, dict), type(batch)
