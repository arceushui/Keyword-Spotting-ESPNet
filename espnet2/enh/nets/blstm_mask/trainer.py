...
from nets.blstm_mask.dataset import logger
...
class PITrainer(object):
    ...
    def train(self, dataset):
        ...
        for input_sizes, nnet_input, source_attr, target_attr in dataset:
            num_batch += 1
            nnet_input = (
                packed_sequence_cuda(nnet_input)
                if isinstance(nnet_input, PackedSequence)
                else nnet_input.to(device)
            )
            ...
            #logger.info(f"Network train set input is {nnet_input}")
            #logger.info(f"Network train set output mask is {masks}")
            cur_loss = self.permutate_loss(masks, input_sizes, source_attr, target_attr)
            ...
            if self.clip_norm:
                th.nn.utils.clip_grad_norm_(self.nnet.parameters(), self.clip_norm)
            self.optimizer.step()
    ...
    def validate(self, dataset):
        ...
        with th.no_grad():
            for input_sizes, nnet_input, source_attr, target_attr in dataset:
                ...
                #logger.info(f"Network train set input is {nnet_input}")
                #logger.info(f"Network train set output mask is {masks}")
                #logger.info(f"Network train set source_attr is {source_attr}")
                #logger.info(f"Network train set target_attr is {target_attr}")
                cur_loss = self.permutate_loss(
                    masks, input_sizes, source_attr, target_attr
                )
                tot_loss += cur_loss.item()
        ...
        for epoch in range(1, num_epoches + 1):
            ...
            logger.info(
                ...
                " dev = {:.4f}({:.2f}s/{:d})".format(
                    epoch,
                    train_loss,
                    on_valid_start - on_train_start,
                    train_num_batch,
                    valid_loss,
                    on_valid_end - on_valid_start,
                    valid_num_batch,
                )
            )
            save_path = os.path.join(self.checkpoint, "epoch.{:d}.pkl".format(epoch))
    ...
    def permutate_loss(self, masks, input_sizes, source_attr, target_attr):
        ...
        logger.info(f"input_sizes is {input_sizes}")
        ...
        def loss(permute):
            ...
            for s, t in enumerate(permute):
                ...
                refer_spect = (
                    targets_spect[t] * F.relu(th.cos(mixture_phase - targets_phase[t]))
                    if is_loss_with_psm
                    else targets_spect[t]
                )
                # N x T x F => N x 1
                utt_loss = th.sum(
                    th.sum(th.pow(masks[s] * mixture_spect - refer_spect, 2), -1), -1
                )
        ...
        # P x N
        pscore = th.stack([loss(p) for p in permutations(range(self.num_spks))])
