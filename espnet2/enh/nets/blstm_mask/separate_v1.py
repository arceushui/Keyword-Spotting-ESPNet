...
from nets.blstm_mask.utils import (
    stft,
    istft,
    parse_scps,
    apply_cmvn,
...
from nets.unet.utils.types import str2bool
...
class Separator(object):
    def __init__(self, nnet, state_dict, cuda=False):
        if not os.path.exists(state_dict):
            raise RuntimeError("Could not find state file {}".format(state_dict))
        self.nnet = nnet
    ...
    def seperate(self, spectra, cmvn=None, apply_log=True):
        ...
        input_spectra = (
            np.log(np.maximum(np.abs(spectra), EPSILON))
            if apply_log
            else np.abs(spectra)
        )
        # apply cmvn or not
        input_spectra = apply_cmvn(input_spectra, cmvn) if cmvn else input_spectra
...
def get_parse():
    parser = argparse.ArgumentParser(
        description="Command to seperate single-channel speech using masks generated by neural networks"
    )
    parser.add_argument("--config", type=str, help="Location of training configure files")
    parser.add_argument("--state_dict", type=str, help="Location of networks state file")
    parser.add_argument(
        "--wave_scp", type=str, help="Location of input wave scripts in kaldi format"
    )
    parser.add_argument(
        "--cuda",
        type=str2bool,
    ...
    parser.add_argument(
        ...
        type=int,
        default=16000,
        help="enhancement wave sample rate",
    )
    return parser
...
if __name__ == "__main__":
    ...
    # default: True, but now it is False
    ...
    for key, utt in utt_dict.items():
        try:
            ...
            logging.info(f"Skip utterance {key}... not found")
            continue
        logging.info(f"Processing {num_utts} utterance {key}")
        logging.info(f"Processing {num_utts} utterance sample rate is {args.fs}")
        num_utts += 1
        norm = np.linalg.norm(samps, np.inf)
        spk_mask, spk_spectrogram = separator.seperate(
            stft_mat, cmvn=dict_mvn, apply_log=apply_log
        )
        print("args.dump_dir is ", args.dump_dir)
        print('args.dump_dir type is ', type(args.dump_dir))
        for index, stft_mat in enumerate(spk_spectrogram):
            istft(
                os.path.join(Path(args.dump_dir), "{}.spk{}.wav".format(key, index + 1)),
                stft_mat,
                frame_length=frame_length,
                frame_shift=frame_shift,
                window=window,
            ...
            if args.dump_mask:
                sio.savemat(
                    os.path.join(Path(args.dump_dir), "{}.spk{}.mat".format(key, index + 1)),
                    {"mask": spk_mask[index]},
                )
    logging.info(f"Processed {num_utts} utterance!\n") 
