...
def stft(
    ...
    samps, _ = audio_lib.load(file,sr=fs) # its output sample rate is alway 22050, not is actully sample of audio file
...
def parse_yaml(yaml_conf):
    if not os.path.exists(yaml_conf):
        raise FileNotFoundError("Could not find configure files...{}".format(yaml_conf))
    ...
    if len(config_dict["train_scp_conf"]) != len(config_dict["valid_scp_conf"]):
