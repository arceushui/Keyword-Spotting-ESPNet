...
from nets.blstm_mask.utils1 import parse_scps, stft, apply_cmvn, EPSILON, get_logger
...
class Dataset(object):
    ...
    def _index_by_key(self, key):
        ...
        #return (self.mixture_reader[key], target_list, key)
    ...
    def _index_by_num(self, num):
        ...
        if num >= len(self.keys_list):
            raise IndexError(
                "Index out of dataset, {} vs {}".format(num, len(self.keys_list))
            )
    ...
    def __getitem__(self, index):
        ...
        """
        if type(index) == int:
            logger.info(f"via int index {index}")
            return self._index_by_num(index)
        elif type(index) == str:
            logger.info(f"via str index {index}")
            return self._index_by_key(index)
        elif type(index) == list:
            logger.info(f"via list index {index}")
...
class BatchSampler(object):
    def __init__(self, sampler_size, batch_size=16, shuffle=True, drop_last=False):
        if batch_size <= 0:
            raise ValueError("Illegal batch_size(= {}) detected".format(batch_size))
    ...
    def __iter__(self):
        ...
        while True:
            if base + step > self.sampler_size:
                break
            yield (
                self.sampler_index[base : base + step]
                if step != 1
                else self.sampler_index[base]
            )
            base += step
...
class DataLoader1(object):
    ...
    def _process(self, index):
        ...
        def prepare_target(dict_list, index, key):
            return pad_sequence(
                [d["target_attr"][key][index] for d in dict_list], batch_first=True
            )
        #for s, t in self.dataset[index]:
        #    logging.info(f's is {s} and t is {t}')
        ...
        target_attr["spectrogram"] = [
            prepare_target(dict_list, s, "spectrogram") for s in range(self.num_spks)
        ]
        ...
        if "phase" in dict_list[0]:
            ...
            target_attr["phase"] = [
                prepare_target(dict_list, s, "phase") for s in range(self.num_spks)
            ]
    ...
    def __iter__(self):
        ...
        for e, index in enumerate(sampler):
            ...
            logger.info(
                    "Processed {} batches, {} utterances uttid is {} ".format(e + 1, num_utts, index)
            )
